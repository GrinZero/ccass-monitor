...
import type { WindowAnalysis, WindowDataPoint, WindowSummary, DailyHoldingsRow } from './types/index.js';

/**
 * 获取指定窗口天数的历史数据
 */
function _getWindowData(stockCode: string, participantId: string, windowDays: number): DailyHoldingsRow[] {
  const now = new Date();
 ...
 * 主入口：分析 N 日窗口
 */
async function analyzeWindow(stockCode: string, participantId: string, windowDays: number = 7): Promise<WindowAnalysis | null> {
  const now = new Date();
  const endDate = formatDate(now);
  const startDate = formatDateNTradingDaysAgo(now, windowDays - 1);
  ...
  const data: WindowDataPoint[] = records.map((r, i) => ({
    date: r.date,
 ...
 * 计算每日变化量
 */
function computeDeltas(records: HoldingRecord[]): number[] {
 ...
 * 计算动量分数（连续同向天数占比）
 */
function computeMomentum(deltas: number[]): number {
 ...
 * 获取某日全体参与者排名
 */
async function getRanking(stockCode: string, date: string): Promise<DailyHoldingsRow[]> {
 ...
 * 获取某参与者平均排名
 */
function computeAvgRank(rankingHistory: Array<{ rank?: number }>): number | null {
